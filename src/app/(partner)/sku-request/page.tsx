...
export default function SkuRequestPage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      // Pre-select service from ?service= query param (passed from SKU dashboard)
  ...
  if (done) {
    ...
    return (
      <div className="max-w-lg mx-auto text-center py-16 px-4">
        ...
        <p className="text-brand-sub text-sm mb-8">
          Admin will review and add units to your account within 24 hours.
        </p>
        <div className="inline-flex items-center gap-3 px-5 py-3 rounded-2xl border mb-8"
          ...
          <div className="text-left">
            ...
            <p className="text-xs" style={{ color: plan.color + 'aa' }}>
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      {/* Header — shows pre-selected service name if coming from dashboard */}
      <div>
        <h1 className="text-xl font-bold text-brand-text">Request SKU</h1>
        <p className="text-brand-sub text-sm mt-1">
          {preselect
            ? `Requesting units for ${preselect}`
            : 'Choose your plan and upload payment proof'}
        </p>
      </div>

      <form onSubmit={handleSubmit} className="space-y-5">
        {error && (
          <p className="bg-red-50 text-red-600 text-sm rounded-xl px-4 py-2">{error}</p>
        )}
            ...
            {PLANS.map(plan => {
              ...
              return (
                <button key={plan.name} type="button" onClick={() => setSelectedPlan(plan.name)}
                  ...
                  style={{ background: active ? plan.bg : '#fff', borderColor: active ? plan.color : '#DDE3EE' }}>
        ...
        <div className="bg-white rounded-2xl p-5 border border-brand-border shadow-card">
          <p className="text-brand-blue text-xs font-bold uppercase tracking-wide mb-3">
            Payment Screenshot
          </p>
        ...
        <button type="submit" disabled={submitting || !imageFile}
          className="w-full py-3.5 rounded-xl bg-brand-blue text-white font-bold text-sm hover:bg-brand-blueDark transition-colors disabled:opacity-50 flex items-center justify-center gap-2">
          {submitting
            ? (
              <>
